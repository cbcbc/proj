...
public class InitController {
    ...
    public String sellerInit(HttpServletRequest request, Model model) {
        ...
        Integer pageId = Integer.parseInt(request.getParameter("pageId"));
        String category = "(1,2,3,4,0)";
        List<Commodity> commoditys = commodityService.getSomeCommoditysByPageId(pageId, sellerId, 10, category);
        model.addAttribute("pageId", pageId);
        model.addAttribute("commoditys", commoditys);
        model.addAttribute("startId",pageId * 10 + 1);
        model.addAttribute("pageNumbers", commodityService.getCommodityPageNumbers(sellerId,10, category));
        return "seller/index";
    }
    ...
    public String ordinaryUserInit(HttpServletRequest request, Model model) {
        ...
        List<Commodity> commoditys =
                commodityService.getSomeCommoditysByPageId(pageId, 1, 6, category); //1表示供应商id
        model.addAttribute("pageId", pageId);
        model.addAttribute("commoditys", commoditys);
        model.addAttribute("startId",pageId * 6 + 1);
        model.addAttribute("category", category);
        model.addAttribute("pageNumbers", commodityService.getCommodityPageNumbers(1, 6, category));
